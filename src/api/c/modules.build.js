...
function buildModule(module) {
  ...
  const exportedFunctions = findKeepaliveFunctions(
    module.srcDir,
    module.incDir,
  );
  ...
  const emccCommand = [
    ...
    "-s",
    `EXPORTED_RUNTIME_METHODS=['ccall','cwrap','UTF8ToString','stringToUTF8','lengthBytesUTF8','setValue','getValue','HEAPU8']`,
    "-s",
  ...
  try {
    console.log(
      `Source files: ${sourceFiles.map((f) => path.basename(f)).join(", ")}`,
    );
...
function findKeepaliveFunctions(srcDir, incDir) {
  ...
  function scanFile(filePath) {
    try {
      ...
      const keepaliveMatches = content.match(
      ...
      const asteriskMatches = content.match(
...
function main() {
  ...
  console.log(
    `Found ${modules.length} module(s): ${modules.map((m) => m.name).join(", ")}`,
  );

  const args = process.argv.slice(2);
  const modulesToBuild =
    args.length > 0 ? modules.filter((m) => args.includes(m.name)) : modules;
